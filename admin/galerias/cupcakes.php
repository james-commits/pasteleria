...
if (isset($_FILES['image']) && isset($_POST['name'])) {
  ...
  if ($image['error'] === UPLOAD_ERR_OK) {
    $directory = '../../img/cupcakes';
    $imagePath = $directory . '/' . $image['name'];
    move_uploaded_file($image['tmp_name'], $imagePath);

    // Lee el contenido del archivo JSON
    $data = [];
    if (file_exists('../../js/cupcakes.json')) {
      $json = file_get_contents('../../js/cupcakes.json');
    ...
    file_put_contents('../../js/cupcakes.json', json_encode($data));
    
 
    header("Location: cupcakes.html");
    echo "Exito";
  } else {
