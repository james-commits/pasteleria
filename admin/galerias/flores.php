...
if (isset($_FILES['image']) && isset($_POST['name'])) {
  ...
  if ($image['error'] === UPLOAD_ERR_OK) {
    $directory = '../../img/flores';
    $imagePath = $directory . '/' . $image['name'];
    move_uploaded_file($image['tmp_name'], $imagePath);

    // Lee el contenido del archivo JSON
    $data = [];
    if (file_exists('../../js/flores.json')) {
      $json = file_get_contents('../../js/flores.json');
    ...
    file_put_contents('../../js/flores.json', json_encode($data));
